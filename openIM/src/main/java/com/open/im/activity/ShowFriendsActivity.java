...
public class ShowFriendsActivity extends Activity {
	...
	private ShowFriendsActivity act;
	private List<String> onlineFriendNames;
	private List<String> offlineFriendNames;
...
//					MyPubSubUtils.unSubscribeFriend(friendName + "@" + connection.getServiceName());
		...
		lv_show_friends.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				...
				String log = converterToFirstSpell(firstItem).substring(0, 1).toUpperCase();
	...
	private void init() {
		...
		lv_show_friends = (ListView) findViewById(R.id.lv_show_friends);
	...
	/**
	 * 根据组名获取组内好友并显示 通过组名获得组 通过组 获得组内的所有好友
	 */
	private void initData() {
		...
		onlineFriendNames = new ArrayList<String>();
		offlineFriendNames = new ArrayList<String>();
		...
		for (RosterEntry rosterEntry : users) {
			...
			// VCard vCard = vCardManager.loadVCard(rosterEntry.getUser());
