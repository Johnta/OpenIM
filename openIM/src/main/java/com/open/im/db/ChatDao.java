...
//		ctx.getContentResolver().notifyChange(uri, null);
    ...
    public void deleteAllVcard() {
        SQLiteDatabase db = helper.getWritableDatabase();
        db.execSQL("delete from " + DBcolumns.TABLE_VCARD);
    }
...
//		ctx.getContentResolver().notifyChange(uri, null);
     ...
     * @param vCardBean
     */
    public void replaceVCard(VCardBean vCardBean) {
        SQLiteDatabase db = helper.getWritableDatabase();
        ContentValues values = new ContentValues();
    ...
    /**
     * 根据jid查询数据库 获取指定jid的vCard信息
     *
     * @param jid
     * @return
     */
    public VCardBean queryVCard(String jid) {
        if (jid == null) {
            return null;
        }
