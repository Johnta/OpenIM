	...
	public void onClick(View v) {
		...
		case R.id.btn_savepicture:
			...
			try {
				...
				MyUtils.showToast(act, "图片保存成功" + imagePath);
				//设置开启这个界面的activity的回调结果
				setResult(RESULT_OK, getIntent().setData(Uri.parse(imagePath)));
				//必须加上这一句，setResult是在这个页面关闭时，将结果发给开启这个界面forresult的activity进行处理的
				finish();
//				MyFileUtils.uploadFile(imagePath);
				bitmap = null;
				try {
	...
	private void acquireWakeLock() {
		if (wakeLock == null) {
			...
			wakeLock = powerManager.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK, "lzh");
			wakeLock.acquire();
		}
	}
